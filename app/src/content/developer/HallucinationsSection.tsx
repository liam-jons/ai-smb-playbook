...
const patterns: AntiHallucinationPattern[] = [
  {
    ...
    whenToUse: 'Any task that involves more than one file, more than one logical concern, or more than roughly 100 lines of change.',
    explanation: 'Claude performs best when given focused, well-scoped tasks. Large, ambiguous requests invite hallucinated approaches because Claude tries to handle too many decisions at once. Breaking work into atomic subtasks means each task gets Claude\u2019s full 200k token context window, and each output is small enough to review meaningfully.',
    crossTrack: true,
    crossTrackNote: 'This works outside coding too \u2014 breaking a long document into sections, handling one email at a time, or tackling one aspect of a problem before moving to the next.',
...
List the subtasks in dependency order. Do not implement anything yet.`,
    ...
    promptDescription: 'Break a complex task into focused, independently testable subtasks.',
  },
  {
    number: 2,
    ...
    crossTrack: true,
    crossTrackNote: 'This works just as well outside of coding \u2014 ask Claude to outline its plan before writing a report, drafting a policy, or preparing meeting notes.',
...
Do not write any code yet. Present the plan and wait for my feedback.`,
    ...
    promptDescription: 'Force Claude to reason about the approach before writing any code.',
  },
  {
    number: 3,
    title: 'Ask for Recommendations and Options',
    whenToUse: 'Architecture decisions, library choices, refactoring approaches, database schema changes, or any decision with multiple valid paths.',
...
Do not recommend one yet \u2014 just present the options so I can evaluate them.`,
    ...
    promptDescription: 'Get Claude to present multiple approaches instead of jumping to one solution.',
  },
  {
    number: 4,
    title: 'Prioritise Best Practice',
    whenToUse: 'Whenever you are working on production code, especially in areas with compliance implications (safeguarding data, authentication, data protection).',
...
If the best-practice approach differs significantly from our current implementation, explain what we currently have wrong and why the change matters. Do not apply quick patches to the existing code \u2014 propose the correct approach even if it requires more work.`,
    ...
    promptDescription: 'Set a high quality bar and name specific standards Claude should follow.',
  },
  {
    number: 5,
    title: 'Give Claude an "Out" (Permit "I Don\u2019t Know")',
    whenToUse: 'Any question about specific API versions, library compatibility, platform-specific behaviour, or configuration details that Claude might not have reliable training data for.',
    ...
    crossTrack: true,
    crossTrackNote: 'This is valuable for general use too \u2014 especially when asking Claude about specific company policies, legal requirements, or factual claims.',
...
With that caveat, how would you approach this?`,
    ...
    promptDescription: 'Give Claude permission to admit uncertainty instead of hallucinating.',
  },
  {
    number: 6,
    title: 'Outline Open Questions Before Implementing',
    whenToUse: 'Any task that spans multiple systems, involves unfamiliar APIs, requires integration work, or has ambiguous requirements.',
    explanation: 'For larger tasks, ask Claude to identify all the open questions and unknowns first, then answer each one, and only then move to implementation. This three-step process prevents Claude from making silent assumptions that lead to hallucinated code paths.',
    crossTrack: false,
...
Now, based on these confirmed details, outline the implementation approach. If any of my answers have created new questions, raise those before proceeding to code.`,
  },
  {
    ...
    whenToUse: 'Any time you are modifying existing code, especially with custom frameworks, internal libraries, or non-standard project structures.',
...
Then propose the new endpoint following the exact same patterns, naming conventions, and project structure as the existing code. Cite the specific files and patterns you are following.`,
    ...
    promptDescription: 'Ground Claude in your actual codebase before it writes new code.',
  },
];
...
const harnessSteps = [
  { label: 'Scope', pattern: 'Pattern 1', description: 'Break the task into atomic subtasks' },
  { label: 'Research', pattern: 'Pattern 6', description: 'Identify open questions for each subtask' },
  { label: 'Resolve', pattern: 'Pattern 5', description: 'Answer questions, giving Claude explicit "outs"' },
  { label: 'Plan', pattern: 'Patterns 2 + 3', description: 'Ask for options and a recommended approach' },
  { label: 'Validate', pattern: 'Patterns 4 + 7', description: 'Check plan against existing code and best practices' },
  { label: 'Implement', pattern: '\u2014', description: 'Only now does Claude write code' },
  { label: 'Review', pattern: '\u2014', description: 'Claude reviews its output against the plan' },
];
...
export function HallucinationsSection() {
  return (
    <div className="flex flex-col gap-12">
      {/* Mini-nav */}
      <nav aria-label="Section navigation" className="rounded-lg border border-border bg-muted/30 p-4">
        ...
        <ol className="columns-1 gap-x-6 space-y-1 text-sm sm:columns-2">
          {patterns.map((p) => (
            <li key={p.number}>
              <a href={`#pattern-${p.number}`} className="text-muted-foreground transition-colors hover:text-foreground">
                {p.number}. {p.title}
              </a>
            </li>
          ))}
          <li>
            <a href="#agent-harness" className="text-muted-foreground transition-colors hover:text-foreground">
              The Agent Harness
            </a>
          </li>
        </ol>
      ...
      {patterns.map((pattern, index) => (
        <PatternCard key={pattern.number} pattern={pattern} isLast={index === patterns.length - 1} />
      ))}
      ...
      <section aria-labelledby="agent-harness">
        <h2 id="agent-harness" className="mb-4 text-xl font-semibold tracking-tight sm:text-2xl">
          The Agent Harness
        </h2>

        <CalloutCard variant="important" className="mb-6">
          The agent harness combines multiple patterns into a structured, multi-step workflow for complex
          development work. Use it for substantial tasks: new features, major refactors, system integrations.
          The individual patterns above are useful on their own for smaller tasks.
        </CalloutCard>
          ...
          {harnessSteps.map((step, index) => (
            <div key={step.label} className="flex items-start gap-3">
              <div className={cn(
                ...
                index < 5 ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
              )}>
                {index + 1}
              </div>
              <div className="flex flex-1 items-start gap-2 pt-0.5">
                ...
                {step.pattern !== '\u2014' && (
                  <Badge variant="outline" className="text-xs">{step.pattern}</Badge>
                )}
                <span className="text-sm text-muted-foreground">&mdash; {step.description}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section aria-labelledby="takeaways">
        <h2 id="takeaways" className="mb-4 text-xl font-semibold tracking-tight sm:text-2xl">
          Key Takeaways
        </h2>
...
function PatternCard({ pattern, isLast }: { pattern: AntiHallucinationPattern; isLast: boolean }) {
  ...
  return (
    <section aria-labelledby={`pattern-${pattern.number}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            <h3 id={`pattern-${pattern.number}`} className="text-lg font-semibold">
              {pattern.title}
            </h3>
            {pattern.crossTrack && (
              <Badge variant="secondary" className="text-xs">Also useful for General users</Badge>
            )}
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            <strong className="text-foreground">When to use:</strong> {pattern.whenToUse}
          </p>
        </div>
      </div>
      ...
      {pattern.followUpPrompt && (
        <Collapsible open={followUpOpen} onOpenChange={setFollowUpOpen} className="mt-3">
          <CollapsibleTrigger className="flex items-center gap-1 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
            <ChevronDown className={cn('h-4 w-4 transition-transform', followUpOpen && 'rotate-180')} />
