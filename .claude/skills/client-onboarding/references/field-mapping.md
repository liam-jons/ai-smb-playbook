...
|-------|----------|----------------------|---------------------------|-------|
| `appTitle` | Yes | No | `"{companyShortName} AI Playbook"` | Derive automatically from `companyShortName`. If `companyShortName` contains trailing punctuation (e.g. `!`), include it in `appTitle` but strip it from `emailSubjectPrefix` and `welcomeSubtitle`. |
...
| `industryContext` | Recommended | Partially | Infer from `industry` | Short descriptor: "design agency", "engineering firm", "law practice". Prioritise the company's actual business activity over the registered company name (e.g. if the company name says "Design" but the business is software development, use "software company"). Always confirm with consultant. |
...
| `database` | Dev only | Sometimes | None | E.g. "SQL Server", "PostgreSQL". Rarely in public-facing content or transcripts. Ask the consultant explicitly rather than inferring from the tech stack. |
...
| `certificationName` | Domain | Sometimes — governance | None | E.g. "DBS checks", "ISO 27001", "SOC 2". This is the sector-level compliance requirement, not the company's own certifications. For safeguarding clients, consider "DBS checks"; for healthcare, "CQC registration"; for financial, "FCA authorisation". Ask the consultant if not evident from transcript. |
